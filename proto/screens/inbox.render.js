...
function renderPipelineReal(el,P){
  const cols=PSTAGES.map(([k,l])=>{
    ...
    const cards=items.map(d=>`<div class="pcard" onclick="showView('inbox')"><div class="pc-top"><span class="pc-name">${esc(d.name||'Unknown')}</span>${d.owner?`<span class="oav-sm" style="background:${d.owner.color}">${esc(d.owner.init)}</span>`:''}</div><div class="pc-co">${esc(d.company||'—')}</div><div class="pc-val">$${(d.value_usd||0).toLocaleString()}${d.prob!=null?`<span> · ${d.prob}%</span>`:''}</div></div>`).join('')||`<div class="pcol-empty">No deals</div>`;
    return `<div class="pcol ${k}"><div class="pcol-h"><span class="pcol-t">${l}</span><span class="pcol-c">${items.length}</span><span class="pcol-sum">$${sum.toLocaleString()}</span></div><div class="pcol-body">${cards}</div></div>`;
  }).join('');
...
function renderAnalytics(){
  ...
  const kpis=[
    ...
    kpi(Math.round(k.replies/k.leads*100)+'%','Reply rate',`${k.replies} replies`),
  ...
  const smax=Math.max(...A.spendByChannel.map(x=>x.v));
  ...
  const dmax=Math.max(...A.leadsByDay);
