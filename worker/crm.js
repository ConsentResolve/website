...
function renderColdEmail(){var box=document.getElementById("coldEmailCard");if(!box)return;api("/api/crm/instantly?funnel=1&campaign=db0041db-080e-4d33-9816-4e66ddd9239c&utm=hvac_2026&trade=hvac").then(function(r){return r.json();}).then(function(d){if(!d||!d.funnel){box.style.display="none";return;}var f=d.funnel;var ins=f.instantly||{};box.style.display="block";var stage=function(label,val,note){return '<div style="flex:1;min-width:74px;text-align:center"><div style="font-size:19px;font-weight:800">'+(val==null?"—":val)+'</div><div class="muted tiny">'+label+'</div>'+(note?'<div class="muted tiny" style="opacity:.65">'+note+'</div>':'')+'</div>';};var arrow='<div style="align-self:center;color:#475569">→</div>';var row=[stage("Leads",ins.leads),arrow,stage("Replied",ins.replied),arrow,stage("Landed",f.landed,"first-party"),arrow,stage("Demos",f.demos,"trade"),arrow,stage("Won",f.won),arrow,stage("Revenue",f.revenueUsd?money(f.revenueUsd):"$0")].join("");box.innerHTML='<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:10px"><div style="font-weight:700">Cold Email → Revenue <span class="muted tiny" style="font-weight:400">· HVAC wave</span></div><span class="muted tiny">'+(f.repliesInbox||0)+' replies in inbox</span></div><div style="display:flex;gap:6px;flex-wrap:wrap">'+row+'</div>'+(!d.configured?'<div class="muted tiny" style="margin-top:8px">Set INSTANTLY_API_KEY in Cloudflare to populate send/reply stats.</div>':'');}).catch(function(){box.style.display="none";});}
...
var composer='<div style="margin-top:14px"><textarea id="ibReply" rows="3" placeholder="Reply to '+esc(who)+(chHint[c.channel]||"")+'…" style="width:100%"></textarea><div style="margin-top:8px"><button class="btn" id="ibSend">Send</button> <span class="muted tiny" id="ibSendMsg"></span></div></div>';
