...
//
// Leads land as a prospect (status='new') for manual Process/triage like every
// other Prospecting-tab source — they do NOT auto-open a CRM inbox conversation.
// That auto-promotion was tried (2026-08) and reverted 2026-08-31: Gojiberry's
// LinkedIn signal volume is high enough that it flooded the Open inbox (339 of
// 388 Open conversations were Gojiberry noise), burying real replies/leads.
// DataForSEO scoring still runs in the background for context either way.
//
...
export async function onRequestPost({ request, env, ctx }) {
  ...
  // NOTE: this used to auto-promote straight to an Open conversation (disposition='open'
  // + executeDisposition). Reverted 2026-08-31 — it flooded the Open inbox (339 of 388
  // Open conversations were Gojiberry noise). The lead now lands as a plain prospect
  // (status='new') for manual Process/triage, same as a DataForSEO sweep or CSV import.

  // Drain the waterfall in the background so this lead's score/tier fill in for context.
  ctx.waitUntil((async () => {
    for (let i = 0; i < 4; i++) {
  ...
  return json({ ok: true, prospect_id: prospectId, domain, existing: !!existing,
    message: "Saved to Prospecting for triage." }, {}, cors);
}
