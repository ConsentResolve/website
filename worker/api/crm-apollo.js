// Apollo.io identified-visitor webhook -> CRM lead, flagged consent_status="identified"
// (cookie-consented to tracking, but not opted into outreach) so the CRM blocks it from
// outreach stages — retargeting/intel only. Register the URL (with ?key=<CRM_WEBHOOK_TOKEN>)
// wherever Apollo can push visitors (native webhook / Zapier / Make). Always 200.
...
export async function onRequestPost({ request, env }) {
  ...
  try {
    ...
    const company = String(deepFind(body, ["company_name", "company", "organization", "organization_name"], 0) || "");
    ...
    const id = await upsertLead(env, {
      source: "apollo", email, name, company, consent_status: "identified",
      notes: linkedin ? "LinkedIn: " + linkedin : null,
    });
    await addActivity(env, id, "identified", "Identified via Apollo" + (company ? " · " + company : "") + (city ? " · " + city : ""), "apollo");
