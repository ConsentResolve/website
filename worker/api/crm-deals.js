...
export async function onRequestGet({ request, env }) {
  ...
  const deals = (await env.DB.prepare(
    `SELECT d.id, d.title, d.value_cents, d.close_probability, d.expected_close_date, d.lead_status,
...
export async function onRequestPost({ request, env }) {
  ...
  if (b.lead_status === "won" || b.lead_status === "lost") { sets.push("won_lost_at=?"); vals.push(new Date().toISOString()); }
  ...
  const action = b.lead_status === "won" ? "won" : b.lead_status === "lost" ? "lost" : b.close_probability !== undefined ? "prob_changed" : "updated";
  await addActivityV2(env, { actorId: actor, entityType: "deal", entityId: b.id, action });
