...
Promise.all([
  ...
  // headline funnel
  $('#kpis').innerHTML=[['Views',views,'social reels (FB/IG/YT)'],['Clicks',clicks,'→ '+pf(clicks,views)+' click-thru'],
  ...
  // views by platform
  const bp={};m.forEach(r=>{bp[r.platform]=(bp[r.platform]||0)+(+r.views||0);});
  $('#byplat').innerHTML=Object.keys(bp).length?'<div class="cards">'+Object.entries(bp).sort((x,y)=>y[1]-x[1]).map(([k,v])=>`<div class="stat"><div class="n">${n(v)}</div><div class="l">${k.toUpperCase()} views</div></div>`).join('')+'</div>':'<div class="empty">No view data yet.</div>';
