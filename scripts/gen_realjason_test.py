#!/usr/bin/env python3
"""One-off: a full UGC-style test reel with the 'Real Jason' avatar, using his
REAL cloned voice (9d5497…, "Aaron Phillips"). That clone has emotion_support=
false, so we OMIT the emotion field and pace with speed + natural pauses.
Uses a new vertical look (fills 9:16, no letterbox) + finish_variants. Review only.
"""
...
LOOK = "b8cf5419ad5247d38bb000fd0df239a6"          # Grey-Goatee Pickup Driver (vertical anchor)
VOICE = "9d5497bed1f144049861da9389addc96"         # Real Jason's actual cloned voice
ANGLE, NAME = "leak", "realjason-leak"
BUILD = ROOT / "build/sprint/realjason"; BUILD.mkdir(parents=True, exist_ok=True)
...
def submit(text, speed):  # no emotion — this real voice clone doesn't support it
